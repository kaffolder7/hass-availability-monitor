...
send_notification() {
  ...
  if [[ -z "$url" ]]; then
    log "ERROR" "$platform URL not configured. Skipping $platform notification."
    return 1
  fi
...
send_telegram_notification() {
  ...
  #   log "ERROR" "Telegram configuration incomplete. Unable to send notification."
  # fi
  ...
  if [[ -z "$NOTIFICATIONS_SERVICES_TELEGRAM_CHAT_ID" || -z "$NOTIFICATIONS_SERVICES_TELEGRAM_BOT_API_TOKEN" ]]; then
    log "ERROR" "Telegram chat ID & Bot token incomplete. Skipping Telegram notification."
    return 1
  fi
...
send_pagerduty_notification() {
  ...
  #   log "ERROR" "PagerDuty configuration incomplete. Unable to send notification."
  # fi
#       curl "${CURL_BASE_OPTIONS[@]}" -X POST -H "Content-Type: application/json" \
...
#       log "ERROR" "Failed to send PagerDuty notification."
  if [[ -z "$NOTIFICATIONS_SERVICES_PAGERDUTY_ROUTING_KEY" ]]; then
    log "ERROR" "PagerDuty routing key not configured. Skipping PagerDuty notification."
    return 1
  fi
...
send_sms() {
  ...
  if [[ "$NOTIFICATIONS_SMS_METHOD" == "twilio" ]]; then
    curl "${CURL_BASE_OPTIONS[@]}" -X POST "https://api.twilio.com/2010-04-01/Accounts/$NOTIFICATIONS_SMS_SETTINGS_TWILIO_ACCOUNT_SID/Messages.json" \
      ...
      log "ERROR" "Failed to send SMS via Twilio to $to_phone."
    ...
    curl "${CURL_BASE_OPTIONS[@]}" -X POST https://api.forwardemail.net/v1/emails \
      ...
      log "ERROR" "Failed to send SMS via ForwardEmail.net to $to_phone."
  fi
}
...
retry_send_sms() {
  ...
  log "ERROR" "Failed to send SMS to $to_phone after $retry_count attempts."
  return 1
}
...
send_email() {
  ...
  if [[ "$NOTIFICATIONS_EMAIL_DRIVER" == "smtp" ]]; then
    ...
    if [[ -z "$NOTIFICATIONS_EMAIL_SETTINGS_SENDGRID_API_KEY" || -z "$NOTIFICATIONS_EMAIL_FROM" ]]; then
      log "ERROR" "Missing required SendGrid configuration"
      return 1
    fi
...
send_notifications_async() {
  local message="$1"
  {
    ...
    if [[ "$(declare -p NOTIFICATIONS_SMS_RECIPIENTS 2>/dev/null)" =~ "declare -a" ]] && [[ ${#NOTIFICATIONS_SMS_RECIPIENTS[@]} -gt 0 ]]; then
      send_sms_to_multiple "$message" || log "ERROR" "SMS notification failed."
    fi &

    # Slack Notifications
    if [[ -n "$NOTIFICATIONS_SERVICES_SLACK_WEBHOOK_URL" ]]; then
      send_slack_notification "$message" || log "ERROR" "Slack notification failed."
    fi &

    # Teams Notifications
    if [[ -n "$NOTIFICATIONS_SERVICES_TEAMS_WEBHOOK_URL" ]]; then
      send_teams_notification "$message" || log "ERROR" "Teams notification failed."
    fi &

    # Discord Notifications
    if [[ -n "$NOTIFICATIONS_SERVICES_DISCORD_WEBHOOK_URL" ]]; then
      send_discord_notification "$message" || log "ERROR" "Discord notification failed."
    fi &

    # Telegram Notifications
    if [[ -n "$NOTIFICATIONS_SERVICES_TELEGRAM_BOT_API_TOKEN" && -n "$NOTIFICATIONS_SERVICES_TELEGRAM_CHAT_ID" ]]; then
      send_telegram_notification "$message" || log "ERROR" "Telegram notification failed."
    fi &

    # PagerDuty Notifications
    if [[ -n "$NOTIFICATIONS_SERVICES_PAGERDUTY_ROUTING_KEY" ]]; then
      send_pagerduty_notification "$message" || log "ERROR" "PagerDuty notification failed."
    fi &
